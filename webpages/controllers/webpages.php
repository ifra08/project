...
function deleteconf($update_id){
     ...
     if(!is_numeric($update_id>3)){
        redirect('site_security/not_allowed');
    } 
     $this->load->module('site_security');
     $this->site_security->_make_sure_is_admin();
     
     $data['headline'] = "<h1>Delete Page</h1>";
...
function create(){
    ...
    if(($submit=="submit"))
    {
       ...
       $this->form_validation->set_rules('page_headline','Page Headline','required|max_length[240]');
    ...
    if(!is_numeric( $update_id))
    {
        $data['headline'] = "<h1>Create new page</h1>";
    }
    else{
        $data['headline'] = "<h1>Update page details </h1>";
    }
